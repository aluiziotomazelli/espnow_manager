...
protected:
    ...
    void SetUp() override
    {
        espnow_driver = std::make_unique<EspNowDriver>(hal_wifi);
        ...
        ON_CALL(hal_wifi, hal_esp_now_init()).WillByDefault(Return(ESP_OK));
        ON_CALL(hal_wifi, hal_espnow_register_recv_cb(_)).WillByDefault(Return(ESP_OK));
        ON_CALL(hal_wifi, hal_espnow_register_send_cb(_)).WillByDefault(Return(ESP_OK));
        ON_CALL(hal_wifi, wifi_set_channel(_, _)).WillByDefault(Return(ESP_OK));
        ON_CALL(hal_wifi, hal_esp_now_add_peer(_)).WillByDefault(Return(ESP_OK));
    }
...
TEST_F(EspNowDriverTest, InitFailGetWifiMode)
{
    ...
    EXPECT_CALL(hal_wifi, hal_esp_now_init()).Times(0); // Must not call esp_now_init, return imediately
...
{
    ON_CALL(hal_wifi, hal_esp_now_init()).WillByDefault(Return(ESP_FAIL));
    EXPECT_CALL(hal_wifi, hal_espnow_register_recv_cb(_)).Times(0); // Must not call hal_espnow_register_recv_cb
    EXPECT_CALL(hal_wifi, hal_espnow_register_send_cb(_)).Times(0);
    ASSERT_EQ(ESP_FAIL, do_init());
}

TEST_F(EspNowDriverTest, HalEspnowRegisterRecvCbFails)
{
    ON_CALL(hal_wifi, hal_espnow_register_recv_cb(_)).WillByDefault(Return(ESP_FAIL));
    EXPECT_CALL(hal_wifi, hal_espnow_register_send_cb(_)).Times(0); // Must not call hal_espnow_register_send_cb
    ASSERT_EQ(ESP_FAIL, do_init());
}

TEST_F(EspNowDriverTest, HalEspnowRegisterSendCbFails)
{
    ON_CALL(hal_wifi, hal_espnow_register_send_cb(_)).WillByDefault(Return(ESP_FAIL));
...
TEST_F(EspNowDriverTest, HalWifiSetChannelFails)
{
    ...
    EXPECT_CALL(hal_wifi, hal_esp_now_add_peer(_)).Times(0); // Must not call esp_now_add_peer
    ASSERT_EQ(ESP_FAIL, do_init());
}

TEST_F(EspNowDriverTest, HalEspNowAddPeerFails)
{
    ON_CALL(hal_wifi, hal_esp_now_add_peer(_)).WillByDefault(Return(ESP_FAIL));
    ASSERT_EQ(ESP_FAIL, do_init());
}
...
TEST_F(EspNowDriverTest, InitCompletesSuccessfully)
{
    ...
    EXPECT_CALL(hal_wifi, hal_esp_now_init()).Times(1);
    EXPECT_CALL(hal_wifi, hal_espnow_register_recv_cb(_)).Times(1);
    EXPECT_CALL(hal_wifi, hal_espnow_register_send_cb(_)).Times(1);
    EXPECT_CALL(hal_wifi, wifi_set_channel(_, _)).Times(1);
    EXPECT_CALL(hal_wifi, hal_esp_now_add_peer(_)).Times(1);
...
TEST_F(EspNowDriverTest, DeinitSuccess)
{
    ...
    EXPECT_CALL(hal_wifi, hal_esp_now_deinit()).Times(1);
...
{
    EXPECT_CALL(hal_wifi, hal_esp_now_deinit()).Times(1).WillOnce(Return(ESP_FAIL));
