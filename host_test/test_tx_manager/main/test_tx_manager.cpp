...
{
    EXPECT_CALL(freertos_hal, queue_create(_, _)).Times(2); // tx_queue + delivery_queue
...
{
    EXPECT_EQ(ESP_ERR_INVALID_ARG, manager->init(1000, 1, nullptr, 10));
}
...
TEST_F(TxManagerTest, InitQueueCreationFailsReturnsError)
{
    ...
    EXPECT_EQ(ESP_ERR_NO_MEM, manager->init(1000, 1, fake_rx_task, 10));
}
...
TEST_F(TxManagerTest, InitSemaphoreCreationFailsReturnsErrorAndCallsDeinit)
{
    ...
    EXPECT_CALL(freertos_hal, queue_delete(_)).Times(2); // Must be called on deinit()
...
TEST_F(TxManagerTest, DeinitWithoutTaskHandleDoesNotTryToDeleteTask)
{
    ...
    EXPECT_CALL(freertos_hal, queue_delete(_)).Times(2);      // But still tries to delete queue
...
TEST_F(TxManagerTest, NotifyDeliveryFailureCallsTaskNotify)
{
    ...
    EXPECT_CALL(freertos_hal, queue_send_fromISR(_, _, _)).Times(1);
    EXPECT_CALL(freertos_hal, task_notify(_, NOTIFY_DELIVERY_FAILURE, _)).Times(1);
    manager->notify_delivery(ESP_NOW_SEND_FAIL, mac);
    deinit_after_init();
}
...
TEST_F(TxManagerTest, NotifyDeliverySuccessCallsTaskNotify)
{
    ...
    uint8_t mac[6] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
    EXPECT_CALL(freertos_hal, queue_send_fromISR(_, _, _)).Times(1);
    EXPECT_CALL(freertos_hal, task_notify(_, NOTIFY_DELIVERY_SUCCESS, _)).Times(1);
    manager->notify_delivery(ESP_NOW_SEND_SUCCESS, mac);
    deinit_after_init();
}
...
TEST_F(TxManagerTest, NotifyWithoutTaskHandleDoesNotCallTaskNotify)
{
    ...
    manager->notify_delivery(ESP_NOW_SEND_FAIL, nullptr);
    manager->notify_delivery(ESP_NOW_SEND_SUCCESS, nullptr);
    manager->notify_link_alive();
}
