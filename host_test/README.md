...
- The project root directory must be named `espnow_manager`. 
...
The tests are placed in directories for each sub-manager or logic component. To run a specific test, navigate to the test directory and run the following commands:

1. Navigate to the test project directory:
   ```bash
   cd host_test/test_bootstraper
   ```
...
4. Run the executable:
   ```bash
   ./build/test_bootstraper.elf
   ```

### Generating Coverage

After running tests in the test directory, you can generate a coverage report for that test by running the following command (using the custom target defined in `coverage_common.cmake`):
...
This will generate a coverage report in the `coverage` directory of the test project, e.g. `host_test/test_bootstraper/coverage`.


## Unified Testing and Coverage (all tests)

For running all tests at once and generating a single unified coverage report for the `espnow_manager` component:
...
1. `make build_all_tests`
2. `make coverage_clean`
3. `ctest --output-on-failure`
4. `make unified_coverage`
